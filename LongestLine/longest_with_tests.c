...
int main( int argc, char *argv[] ) {		// LLK addition for unit testing
	...
	int longest_len = 0; //keep track of longest line's length
	char longest[MAXLINE + 1]; //store longest line
	...
	copy(longest, ""); //init longest with empty string
	while ( (len = readline( line, MAXLINE )) > 0 ) {
		printf("%s\n", line);
		if (len > longest_len) { //entered line is longer than stored longest
			copy(longest, line);
			longest_len = len;
		}
	}
...
void copy( char to[], char from[] ) {
	...
	int i;
	for (i = 0; i <= from_size; i++) { // use <= to include null terminator
		to[i] = from[i];
	}
}
